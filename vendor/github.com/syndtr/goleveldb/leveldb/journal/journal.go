...
//
// There are four chunk types: whester the chunk is the full journal, or the
...
type Reader struct {
	...
	n int
	// last is whester the current chunk is the last chunk of the journal.
	last bool
...
type Writer struct {
	...
	written int
	// first is whester the current chunk is the first chunk of the journal.
	first bool
	// pending is whester a chunk is buffered but not yet written.
	pending bool
	// err is any accumulated error.
	err error
	// buf is the buffer.
