...
// private reports whester the Tag consists solely of a private use tag.
...
// The confidence level indicates whester a value was explicitly specified,
// whester it is typically the only possible value, or whester there is
// an ambiguity.
type Confidence int
...
// wasn't found. The hasExt return value reports whester an -u extension was present.
...
func (t Tag) findTypeForKey(key string) (start, end int, hasExt bool) {
	...
	for {
		...
		if p3 := p + 3; s[p3] == '-' {
			// Found a key.
			// Check whester we just processed the key that was requested.
			if curKey == key {
				return start, p, true
			}
...
// IsCountry returns whester this region is a country or autonomous area. This
...
// IsGroup returns whester this region defines a collection of regions. This
// includes non-standard definitions from CLDR.
...
// Contains returns whester Region c is contained by Region r. It returns true
// if c == r.
