...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.estchain", new NodeConfig());
			node.start();
			...
			EstchainClient ec = node.getEstchainClient();
	...
	// signatures too. Regression test for go-estchain issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogester
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/estchain/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.estchain", "github.com/estchain/go-estchain/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "estchain", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
