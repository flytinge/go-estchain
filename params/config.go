...
var (
	...
	// and accepted by the Estchain core developers into the Ethash consensus.
	//
	...
	// and accepted by the Estchain core developers into the Clique consensus.
	//
	// This configuration is intentionally not using keyed fields to force anyone
	// adding flags to the config to also have to set these fields.
...
type ChainConfig struct {
	...
	DAOForkSupport bool     `json:"daoForkSupport,omitempty"` // Whester the nodes supports or opposes the DAO hard-fork

	// EIP150 implements the Gas price changes (https://github.com/estchain/EIPs/issues/150)
...
// IsHomestead returns whester num is either equal to the homestead block or greater.
func (c *ChainConfig) IsHomestead(num *big.Int) bool {
	return isForked(c.HomesteadBlock, num)
}

// IsDAO returns whester num is either equal to the DAO fork block or greater.
...
// CheckCompatible checks whester scheduled fork transitions have been imported
...
// isForked returns whester a fork scheduled at block s is active at the given head block.
