...
// Remove deletes a transaction from the maintained map, returning whester the
...
type txList struct {
	strict bool         // Whester nonces are strictly continuous or not
...
// Overlaps returns whester the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whester the
...
// Remove deletes a transaction from the maintained list, returning whester the
...
// Empty returns whester the list of transactions is empty or not.
...
// Underpriced checks whester a transaction is cheaper than (or as cheap as) the
