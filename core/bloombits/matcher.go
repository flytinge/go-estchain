...
type Matcher struct {
	...
	running uint32 // Atomic flag whester a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors togester.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togester
				var orVector []byte
...
// the reuested retrieval queue to be serviced togester with other sessions.
//
