...
// Package utils contains internal helper functions for go-estchain commands.
package utils
...
import (
	...
	"github.com/estchain/go-estchain/core/state"
	"github.com/estchain/go-estchain/core/vm"
	"github.com/estchain/go-estchain/crypto"
	"github.com/estchain/go-estchain/dashboard"
	...
	"github.com/estchain/go-estchain/metrics"
	...
	"github.com/estchain/go-estchain/p2p/netutil"
	"github.com/estchain/go-estchain/params"
	whisper "github.com/estchain/go-estchain/whisper/whisperv5"
...
// RegisterEthService adds an Estchain client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Estchain service: %v", err)
	}
}
...
// RegisterEthStatsService configures the Estchain Stats daemon and adds it to
// th egiven node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Estchain
		ctx.Service(&ethServ)

		var lesServ *les.LightEstchain
		ctx.Service(&lesServ)

		return ethstats.New(url, ethServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Estchain Stats service: %v", err)
	}
}
