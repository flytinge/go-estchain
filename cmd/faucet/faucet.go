// Copyright 2017 The go-estchain Authors
// This file is part of go-estchain.
//
// go-estchain is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// go-estchain is distributed in the hope that it will be useful,
...
import (
	...
	"github.com/estchain/go-estchain/eth"
	"github.com/estchain/go-estchain/eth/downloader"
	"github.com/estchain/go-estchain/ethclient"
	"github.com/estchain/go-estchain/ethstats"
	"github.com/estchain/go-estchain/les"
	"github.com/estchain/go-estchain/log"
	"github.com/estchain/go-estchain/node"
	"github.com/estchain/go-estchain/p2p"
	"github.com/estchain/go-estchain/p2p/discover"
	"github.com/estchain/go-estchain/p2p/discv5"
...
var (
	...
	ethPortFlag = flag.Int("ethport", 31666, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Estchain protocol")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for Estchain and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Estchain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Estchain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Estchain protocol stack
	client *ethclient.Client   // Client connection to the Estchain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Estchain light client protocol
...
// close terminates the Estchain connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Estchain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/estchain/go-estchain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and Estchain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Estchain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Estchain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Estchain address and profile URL.
...
// returning the username, avatar URL and Estchain address to fund on success.
