...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/estchain                                                            && \
	(cd $GOPATH/src/github.com/estchain && git clone --depth=1 https://github.com/estchain/go-estchain) && \
  go build -v github.com/estchain/go-estchain/cmd/faucet                                              && \
...
// checkFaucet does a health-check against an faucet server to verify whester
