...
// nodeDockerfile is the Dockerfile required to run an Estchain node.
var nodeDockerfile = `
FROM estchain/client-go:latest
...
// an Estchain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Estchain node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.estchain"],
