...
func (w *wizard) deployNode(boot bool) {
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{portFull: 31666, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{portFull: 31666, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	...
	if !boot {
		if w.conf.genesis.Config.Ethash != nil {
			// Ethash based miners only need an esterbase to mine against
			fmt.Println()
			if infos.esterbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.esterbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.esterbase)
				infos.esterbase = w.readDefaultAddress(common.HexToAddress(infos.esterbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy Estchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
