...
type Config struct {
	Oracle common.Address // Estchain address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Estchain service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var estchain *eth.Estchain
	if err := ctx.Service(&estchain); err == nil {
		apiBackend = estchain.ApiBackend
	} else {
		var estchain *les.LightEstchain
		if err := ctx.Service(&estchain); err == nil {
			apiBackend = estchain.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/estchain/go-estchain/releases for new releases")
