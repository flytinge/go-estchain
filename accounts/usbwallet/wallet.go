...
import (
	...
	estchain "github.com/estchain/go-estchain"
...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Estchain
...
type wallet struct {
	...
	deriveChain    estchain.ChainStateReader // Blockchain state reader to discover used account with
...
// whester they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Estchain account
...
// Contains implements accounts.Wallet, returning whester a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain estchain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
